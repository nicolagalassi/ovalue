...
const breakdown = computed(() => {
    ...
    if (lfMetal > 0) bonuses.push({ n: 'Ric. LF', v: lfMetal });
    const hasLfResearch = Object.values(p.lfResearch || {}).some(v => v);
    if (!hasLfResearch && (g.lfBonus || 0) > 0) bonuses.push({ n: 'Tech LF', v: g.lfBonus });
    if (actCraw > 0) {
        let mult = 0.02;
...
<template>
  <div class="planet-card bg-[#0d1525] rounded-xl border border-slate-700/25 hover:border-slate-600/40 transition-all duration-200 overflow-hidden flex flex-col">
    ...
    <div class="px-3 pt-2 pb-2 border-b border-slate-700/20 space-y-2">
      ...
      <div class="flex gap-0.5 bg-[#070c18]/50 rounded-lg p-0.5">
      ...
      <div class="flex items-end justify-between gap-2">
        ...
        <div v-if="planet.lifeform === 'rocktal'" class="flex flex-col gap-0.5">
          ...
          <input type="number" v-model.number="planet.magma" @focus="$event.target.select()"
                 class="w-16 bg-[#070c18]/50 border border-orange-500/20 rounded-lg px-2 py-1 text-[11px] font-mono text-orange-300 text-center focus:outline-none focus:border-orange-400/50" />
        </div>
        <div v-else-if="planet.lifeform === 'humans'" class="flex flex-col gap-0.5">
          ...
          <input type="number" v-model.number="planet.human" @focus="$event.target.select()"
                 class="w-16 bg-[#070c18]/50 border border-green-500/20 rounded-lg px-2 py-1 text-[11px] font-mono text-green-300 text-center focus:outline-none focus:border-green-400/50" />
        </div>
    ...
    <div class="border-b border-slate-700/20">
      ...
      <!-- Corpo espandibile -->
      <div v-show="showLfResearch" class="px-3 pb-3">
          ...
          <div class="mt-3 pt-2 border-t border-slate-700/20">
            ...
            <div v-for="b in lfMetalBuildings" :key="b.id"
              ...
              <input type="number" v-model.number="planet.lfBuildings[b.id]" @focus="$event.target.select()" min="0"
                     class="flex-shrink-0 w-12 bg-[#070c18]/60 border border-slate-700/30 rounded-md px-1 py-1 text-[11px] font-mono text-amber-300 text-center focus:outline-none focus:border-amber-400/40" />
