...
(async function () {
    ...
    // ── BRIDGE MODE ──────────────────────────────────────────────────────────
    // Quando lo script gira su ovalue.net copia i dati GM → localStorage
    ...
    // Universe name/speed: prima dai meta tag (sincrono), poi affina con serverData.xml
    ...
    // Cattura lifeform pianeta corrente, con retry (fino a ~3s)
    ...
    // Overview: classe giocatore + ufficiali + lifeform pianeta corrente
    function collectOverview() {
        d.playerName = getPlayerName() || d.playerName;

        const sprite = document.querySelector('#characterclass .sprite');
        if (sprite) {
            if      (sprite.classList.contains('miner'))    d.playerClass = 'Collezionista';
            else if (sprite.classList.contains('warrior'))  d.playerClass = 'Generale';
            else if (sprite.classList.contains('explorer')) d.playerClass = 'Esploratore';
        }

        const officers = {};
        for (const [role, name] of Object.entries(OFFICER_ROLES)) {
            ...
            const timeRemaining = timerEl?.textContent.trim() || 'Permanente';
            officers[name] = { active, timeRemaining };
        }
        d.officers = officers;
    ...
    // LF Bonuses: bonus metallo + bonus classe
    function collectLFBonuses() {
        ...
        document.querySelectorAll('inner-bonus-item-heading').forEach(el => {
            ...
            const bonus = bonusEl.textContent.replace(/Totale:|Total:/gi, '').trim();
            if (!bonus) return;
            if (/^Metallo$|^Metal$/i.test(title))           bonuses.metal     = bonus;
            if (/^Collezionista$|^Collector$/i.test(title)) bonuses.classBonus = bonus;
        });
        d.lfBonuses = bonuses;
        d.lf_collected = true;
        captureLifeform();
    ...
    // Pagina Impero: plasma + pianeti + global items
    function collectEmpire() {
        ...
        // Mappa coords→id dal sidebar (fallback per empire standalone, dove id sul .planet c'è comunque)
        const sidebar = getSidebarPlanets();
        const coordToId = {};
        sidebar.forEach(sp => { if (sp.id && sp.coords) coordToId[sp.coords] = sp.id; });

        // Livello edificio: cerca nel container del pianeta
        ...
        document.querySelectorAll('.planet:not(.summary)').forEach(p => {
            ...
            p.querySelectorAll('.item_img').forEach(img => {
                ...
                if (/metallo Bronzo|Bronze Metal/i.test(t))    item = Math.max(item, 10);
                if (/metallo Argento|Silver Metal/i.test(t))   item = Math.max(item, 20);
                if (/metallo Oro|Gold Metal/i.test(t))         item = Math.max(item, 30);
                if (/metallo Platino|Platinum Metal/i.test(t)) item = Math.max(item, 40);
                const amp = t.match(/(?:Amplificatore di risorse|Resource Amplifier)[^\d]*(\d+)/i);
                if (amp) itemCustom = parseInt(amp[1]);
            });

            planets.push({
                id:        planetId,
                name:      p.querySelector('.planetname')?.textContent.trim() || '',
                coords, pos,
        ...
        // Global items — legge da tutte le .empireItems, deduplicati per nome
        const seen = new Set();
        const globalItems = [];
        document.querySelectorAll('.empireItems .item_img').forEach(img => {
            ...
            if (/Amplificatore|Amplifier|metallo|Metal/i.test(name)) return;
            seen.add(name);

            const html      = pipeIdx >= 0 ? tooltip.slice(pipeIdx + 1) : '';
            const durMatch  = html.match(/Durata rimanente:\s*([^<]+)/i);
            const timeRemaining = durMatch?.[1]?.trim()
                ?? (tooltip.includes('Permanente') ? 'Permanente' : 'Permanente');
    ...
    collectUniverseInfo(); // sempre: aggiorna player + universe info
    ...
    if (page === 'ingame' && component === 'overview') {
        ...
        // Tutte le altre pagine: cattura passiva lifeform (con retry interno)
    ...
    const empLink  = (label) =>
        `<a class="ov_empire_link" href="${empireUrl()}" target="_blank" rel="noopener noreferrer">${label}</a>`;
    ...
    function updatePanel() {
        ...
        if (ovBody) {
            if (!d.overview_collected) {
                ovBody.innerHTML = `<div class="ov_hint">Vai alla <a href="?page=ingame&component=overview">Panoramica</a> e attendi il caricamento.</div>`;
            } else {
                const classColor = { Collezionista: '#6aafdf', Generale: '#df8a6a', Esploratore: '#8adf6a' }[d.playerClass] || '#7a9ab2';
                let html = row('Giocatore', `<span style="color:#a0bcd4">${d.playerName || '—'}</span>`);
                html    += row('Classe',    `<span style="color:${classColor};font-weight:bold">${d.playerClass !== 'none' ? d.playerClass : '—'}</span>`);
                for (const [, name] of Object.entries(OFFICER_ROLES)) {
                    const off = d.officers[name];
                    if (!off) { html += row(`✗ ${name}`, '<span class="ov_dim">—</span>'); continue; }
                    if (!off.active) { html += row(`<span class="ov_dim">✗ ${name}</span>`, '<span class="ov_dim">assente</span>'); continue; }
                    const perm = /^(Permanente|∞)$/i.test(off.timeRemaining);
        ...
        if (lfBody) {
            if (!d.lf_collected) {
                lfBody.innerHTML = `<div class="ov_hint">Vai ai <a href="?page=ingame&component=lfbonuses">Bonus LifeForm</a> e attendi il caricamento.</div>`;
            } else {
                lfBody.innerHTML =
                    row('Bonus Metallo', `<span class="ov_val">${d.lfBonuses.metal}</span>`) +
                    row('Bonus Classe',  `<span class="ov_val">${d.lfBonuses.classBonus}</span>`);
            }
        }
        ...
        if (empBody) {
            if (!d.empire_collected) {
                empBody.innerHTML = `<div class="ov_hint">Vai alla ${empLink('pagina Impero')} e attendi il caricamento.</div>`;
            } else {
                let html = row('Pianeti', `<span class="ov_val">${d.planets.length}</span>`) +
                           row('Plasma',  `<span class="ov_val">Liv. ${d.settings.plasma}</span>`);

                const missing = getMissingPlanets();
                if (missing.length) {
                    html += subTitle(`⚠ Pianeti Mancanti (${missing.length})`);
                    html += `<div class="ov_hint">Visita la ${empLink('pagina Impero')} per leggerli.</div>`;
                    missing.forEach(p => { html += row(`✗ ${p.name || ''} [${p.coords}]`, '—'); });
                }

                const lfMissing = getMissingLifeforms();
                if (lfMissing.length) {
                    html += subTitle(`⚠ Razze Mancanti (${lfMissing.length})`);
                    html += `<div class="ov_hint">Clicca su ciascun pianeta per registrare la specie.</div>`;
                    lfMissing.forEach(p => {
                ...
                // Fonde d.planets con d.planetLifeforms (la cattura passiva aggiorna solo la mappa)
                const knownLf = d.planets
                    .map(p => ({ ...p, lifeform: d.planetLifeforms[p.id] || p.lifeform }))
                    .filter(p => p.lifeform && p.lifeform !== 'none');
                if (knownLf.length) {
                    html += subTitle(`Razze Attive (${knownLf.length})`);
                    knownLf.forEach(p => {
                ...
                if (d.globalItems.length) {
                    html += subTitle(`Item Globali (${d.globalItems.length})`);
                    d.globalItems.forEach(it => {
                        const perm = /permanente/i.test(it.timeRemaining);
        ...
        // Ri-applica target su empire link eventualmente generati dall'innerHTML
        document.querySelectorAll('.ov_empire_link').forEach(a => {
            a.target = '_blank';
    ...
    function injectUI() {
        ...
        panel.innerHTML = `
            <div id="ov_hdr">
                ...
                <button id="ov_close" title="Chiudi">✕</button>
            </div>
            <div id="ov_content">
                <div class="ov_sec">
                    <div class="ov_sec_hdr">
                        <a class="ov_sec_lnk" href="?page=ingame&component=overview">👤 Panoramica</a>
                        <span id="ov_bdg_ov" class="ov_badge ov_ko">✗ MANCANTE</span>
                    </div>
                ...
                <div class="ov_sec">
                    <div class="ov_sec_hdr">
                        <a class="ov_sec_lnk" href="?page=ingame&component=lfbonuses">🧬 LifeForm</a>
                        <span id="ov_bdg_lf" class="ov_badge ov_ko">✗ MANCANTE</span>
                    </div>
                ...
                <div class="ov_sec">
                    <div class="ov_sec_hdr">
                        <a class="ov_sec_lnk ov_empire_link" href="${empireUrl()}" target="_blank" rel="noopener noreferrer">🌍 Impero</a>
                        <span id="ov_bdg_emp" class="ov_badge ov_ko">✗ MANCANTE</span>
                    </div>
                    <div id="ov_body_emp" class="ov_body">
                        <div class="ov_hint">Vai alla <a class="ov_empire_link" href="${empireUrl()}" target="_blank" rel="noopener noreferrer">pagina Impero</a> e attendi il caricamento.</div>
                    </div>
                </div>
                <button id="ov_export">⬇ Esporta Dati OValue</button>
                <button id="ov_reset">🗑 Svuota Cache Universo</button>
            </div>
        `;
        document.body.appendChild(panel);

        // Posiziona pannello a sinistra del menu
        ...
        document.getElementById('ov_export').addEventListener('click', () => {
            // Riallinea lifeform dalla mappa prima dell'export
            d.planets.forEach(p => {
                if (p.id != null && d.planetLifeforms[p.id])
                    p.lifeform = d.planetLifeforms[p.id];
            });
        ...
        document.getElementById('ov_reset').addEventListener('click', () => {
            if (!confirm(`Svuotare la cache per "${SERVER_KEY}"?`)) return;
